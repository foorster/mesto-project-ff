...
export function deleteNewCard(cardId) {
  // Функции удаления карточки передаем ее айди (надо будет проверить айди на собственность)
...
export function putLikeCard(cardId) {
  // Функции лайка надо передать айди карточки
...
export function delDislikeCard(cardId) {
  // Функции дизлайка надо передать айди карточки
  return fetch(`${config.baseUrl}${config.cohortId}/cards/likes/${cardId}`, {
    method: "DELETE",
    headers: config.headers,
  }).then(checkResponse);
...
export function changeAvatar(url) {
  // Передаем функции ссылку аватарки
